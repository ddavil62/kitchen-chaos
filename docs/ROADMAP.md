...
| Phase 18 | 업적 시스템 (30~50개, 보상, 업적 UI) | 📋 계획 |
| Phase 19 | 레거시 정리 + 기술 부채 | 📋 계획 |
| Phase 20 | 시즌 2 기반 인프라 (월드맵 확장, 신규 캐릭터, 세이브) | 📋 계획 |
| Phase 21 | 7장 — 사쿠라 이자카야 (일식) | 📋 계획 |
| Phase 22 | 8장 — 용의 주방 (중식) | 📋 계획 |
| Phase 23 | 9장 — 별빛 비스트로 (양식) | 📋 계획 |
| Phase 24 | 10장 — 향신료 궁전 (인도) | 📋 계획 |
| Phase 25 | 11장 — 선인장 칸티나 (멕시칸) | 📋 계획 |
| Phase 26 | 12장 — 슈가 드림랜드 (디저트 월드, 시즌 2 최종) | 📋 계획 |
| Phase 27 | 시즌 2 밸런스 QA + 스토리 종합 | 📋 계획 |
...
> 목표: 장기 플레이 동기부여, 업적 UI, 업적 보상으로 코인/칭호 지급

### 18-1. 업적 데이터 + AchievementManager

- [ ] achievementData.js: 업적 30~50종 정의 (id, nameKo, descKo, condition, reward)
- [ ] 업적 카테고리: 전투(보스 처치, 누적 킬), 서빙(누적 서빙, 콤보), 수집(레시피, 재료), 진행(챕터 클리어, 별점), 도전(엔드리스 웨이브, 무피해 클리어)
- [ ] AchievementManager.js: check, unlock, getAll, getUnlocked, getProgress
- [ ] SaveManager 버전업: achievements 필드 추가

### 18-2. 업적 UI

- [ ] AchievementScene.js 또는 MenuScene 내 업적 패널
- [ ] 카테고리별 탭, 달성/미달성 구분, 진행률 바
- [ ] 업적 달성 시 토스트 알림 (VFXManager 연동)
- [ ] 보상 수령 UI (코인, 칭호)

### 18-3. 트리거 연동

- [ ] ResultScene: 전투/서빙 관련 업적 체크
- [ ] GatheringScene: 전투 업적 (보스 처치, 무피해 등)
- [ ] ServiceScene: 서빙 업적 (콤보, 누적)
- [ ] ShopScene: 수집 업적 (레시피 해금)

---

## Phase 19 — 레거시 정리 + 기술 부채
...
- [ ] **유키** (❄️) — 일본 출신 미력사, 차분하고 정밀한 칼잡이. 셰프 스킬: 정밀 절단(조리시간 -25%, 90초 CD)
- [ ] **라오** (🐉) — 중국 출신 미력사, 호쾌한 웍 마스터. 셰프 스킬: 불꽃 웍(전 테이블 주문 즉시 완성, 120초 CD)
- [ ] PixelLab 초상화 + 스프라이트 (48px, 8방향)
- [ ] CHARACTERS에 추가, dialogueData.js 캐릭터 정의
- [ ] ChefSelectScene 5종 셰프 대응 (기존 3 + 신규 2)

### 20-2. 월드맵 확장 (12챕터)

- [ ] WorldMapScene: 6노드 → 12노드 확장 (스크롤 또는 페이지 전환)
- [ ] 시즌 1(1~6장) / 시즌 2(7~12장) 시각 구분
- [ ] 7장 해금 조건: 6-3 클리어 + 시즌 2 인트로 대화 완료

### 20-3. 세이브 확장

- [ ] SaveManager 버전업: 시즌 2 진행도, 신규 셰프, 확장 스토리 플래그
- [ ] storyData.js: 시즌 2 프롤로그 트리거 (WCA 연락, 원정 출발)
- [ ] 시즌 2 인트로 대화 스크립트 (미미+포코+WCA 연락원)

### 20-4. 신규 도구 2종

- [ ] **와사비 대포** (wasabi_cannon) — 범위 공격 + 둔화, 일식 테마
- [ ] **향신료 그라인더** (spice_grinder) — 지속 피해(DoT), 다국적 테마
- [ ] TOOL_DEFS 확장 (8종), ToolManager/MerchantScene 대응
- [ ] PixelLab 도구 에셋 2종

---

## Phase 21 — 7장: 사쿠라 이자카야 (일식)
...
- [ ] 재료 2종: 참치(🐟→🍣 sashimi_tuna), 와사비(🌿 wasabi) — 누적 17종
- [ ] 적 2종: sushi_ninja (HP ~250, 은신+백어택), tempura_monk (HP ~300, 배리어)
- [ ] 보스: sake_oni (HP 6000, 취권 패턴, 주변 아군 버프)
- [ ] PixelLab 스프라이트 (32/48px, 8방향, walk)

### 21-2. 스테이지 7-1 ~ 7-6

- [ ] 6스테이지 + 웨이브 구성 (시즌 1 난이도 이어서 상승)
- [ ] 일식 전용 타일셋 (PixelLab 아이소메트릭)
- [ ] 서비스 설정 (인내심 32~22초 구간)

### 21-3. 레시피 확장 (~10종)

- [ ] 서빙 8~9종 (참치+와사비 조합: 사시미, 스시, 와사비롤 등)
- [ ] 버프 1~2종 (일식 테마)
- [ ] 누적 ~116종

### 21-4. 스토리 스크립트

- [ ] chapter7_intro (유키 소개, WCA 미션 브리핑)
- [ ] chapter7_yuki_joins (유키 합류 이벤트)
- [ ] chapter7_clear (사케 오니 격퇴)
- [ ] 사이드 대화 1~2종 (유키 캐릭터 심화)

---

## Phase 22 — 8장: 용의 주방 (중식)
...
## Phase 26 — 12장: 슈가 드림랜드 (디저트 월드, 시즌 2 최종)
...
- 30스테이지, 106레시피, 15재료, 22적, 6도구, 3셰프

### 시즌 2 — 국제 식란 SOS (챕터 7~12, Phase 20~27)
...
| 18 | 업적 시스템 (30~50개, 보상 UI) | 106 | 15 | 📋 계획 |
| 19 | 레거시 정리 + 기술 부채 | 106 | 15 | 📋 계획 |
| 20 | 시즌 2 기반 (월드맵 12챕터, 셰프 2종, 도구 2종) | 106 | 15 | 📋 계획 |
| 21 | 7장 사쿠라 이자카야 (참치, 와사비) | ~116 | 17 | 📋 계획 |
| 22 | 8장 용의 주방 (두부, 고수) | ~126 | 19 | 📋 계획 |
| 23 | 9장 별빛 비스트로 (트러플) | ~136 | 20 | 📋 계획 |
| 24 | 10장 향신료 궁전 (커리잎, 사프란) | ~146 | 22 | 📋 계획 |
| 25 | 11장 선인장 칸티나 (할라피뇨) | ~156 | 23 | 📋 계획 |
| 26 | 12장 슈가 드림랜드 (카카오, 바닐라) — 시즌 2 최종 | ~166 | 25 | 📋 계획 |
| 27 | 시즌 2 밸런스 QA + 스토리 종합 | ~166 | 25 | 📋 계획 |
