...
## Phase 12 — 도구 시스템 리워크 (진행 중)

> 목표: 핵심 경제 루프 분리 — 재료 채집(TD)에서 골드 제거, 타워를 영구 도구로 전환, 행상인 씬 추가
> 상세 기획서: `.claude/specs/2026-04-10-kitchen-chaos-phase12-scope.md`
...
| 서브페이즈 | 내용 | 상태 |
|-----------|------|------|
| **12-1** | 도구 데이터 + ToolManager + 세이브 v9 | 예정 |
| **12-2** | 행상인 씬 (MerchantScene) | 예정 |
| **12-3** | 재료 채집 씬 리워크 (GatheringScene) | 예정 |
| **12-4** | 엔드리스 적용 + 골드 연동 + 밸런싱 | 예정 |

**구현 순서**: 12-1 → 12-2 → 12-3 → 12-4
...
| **12** | **도구 시스템 리워크, 행상인, 재료 채집** | 106 | 15 | 🔧 진행 중 |
