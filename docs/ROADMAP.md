...
| Phase 31 | 16장 — 향신료 궁전 (인도 1장) | 📋 계획 |
| Phase 32 | 17장 — 향신료 궁전 심층부 (인도 2장) | 📋 계획 |
| Phase 33 | 18장 — 향신료 마하라자 최종전 (인도 3장) | 📋 계획 |
| Phase 34 | 19장 — 선인장 칸티나 (멕시칸 1장) | 📋 계획 |
| Phase 35 | 20장 — 칸티나 심층부 (멕시칸 2장) | 📋 계획 |
| Phase 36 | 21장 — 엘 디아블로 최종전 (멕시칸 3장) | 📋 계획 |
| Phase 37 | 22장 — 슈가 드림랜드 (디저트 1장) | 📋 계획 |
| Phase 38 | 23장 — 드림랜드 심층부 (디저트 2장) | 📋 계획 |
| Phase 39 | 24장 — 미각의 여왕 최종전 (최종) | 📋 계획 |
| Phase 40 | 챕터 16~24 밸런스 QA | 📋 계획 |
| Phase 41 | 업적 시스템 (30~50개, 보상, 업적 UI) | 📋 계획 |
| Phase 42 | 아트 리워크 — 레거시 스프라이트 64px 재생성 | 🎨 예정 |

**현재 구현 완성도**: 시즌 1 완성, Phase 21 완료 (시즌2 7~8장 콘텐츠 -- 적 7종+보스 2종+재료 4종+레시피 20종+스토리 8종+은신/배리어/취권/아우라/분열/화염장판/3페이즈 메커닉)
...
| 재료 종류 | 19종 (당근, 고기, 문어, 고추, 밀가루, 달걀, 쌀, 생선, 버섯, 치즈, 새우, 토마토, 버터, 설탕, 우유, 참치, 와사비, 두부, 고수) |
...
## Phase 22 — 8장: 이자카야 심층부 (일식 2장) + 7장 보스 재편

> 테마: 이자카야 지하 비밀 구역. 사케 오니가 있는 곳.
> 스토리: 유키가 지하 비밀 구역을 발견. 7장 보스(사케 오니)를 9장으로 재편.

### 22-1. 7장 보스 재편

- [ ] sake_oni 보스를 9-6으로 이동 (7장 스테이지 7-6은 중간 보스/미니보스로 교체)
- [ ] stageData.js 7-6 웨이브 재설정
- [ ] 7장 보스 격퇴 대화(chapter7_clear) 재조정

### 22-2. 신규 재료/적 (8장)

- [ ] 재료 1종: 사케(sake) — 누적 20종
- [ ] 적 2종: sake_specter (HP ~300, 마취 디버프), oni_minion (HP ~350, 돌진)
- [ ] PixelLab 스프라이트

### 22-3. 스테이지 8-1 ~ 8-5

- [ ] 5스테이지 (8-6은 Phase 23에서 사케 오니)
- [ ] 이자카야 지하 타일셋
- [ ] 서비스 설정

### 22-4. 스토리 스크립트

- [ ] chapter8_intro (유키 지하 단서 발견)
- [ ] chapter8_mid (팀 내 갈등 — 유키 단독 행동 vs 팀 협력)
- [ ] 사이드 대화 1종
...
> 테마: 사케 오니와의 최종 결전. 일식 아크 마무리.
> 스토리: 유키가 오니의 정체(타락한 옛 미력사)를 알게 되며 성장.

### 23-1. 스테이지 9-1 ~ 9-6

- [ ] 5스테이지 + 보스전 9-6
- [ ] 보스: sake_oni (HP 6500, 취기 장판+소환, 3페이즈)
- [ ] PixelLab 보스 스프라이트 (업데이트)

### 23-2. 레시피 확장 (~10종)

- [ ] 서빙 8~9종 (사케+기존 일식 재료 조합)
- [ ] 버프 1~2종
- [ ] 누적 ~136종

### 23-3. 스토리 스크립트

- [ ] chapter9_boss (사케 오니 최종전 전 대사)
- [ ] chapter9_clear (유키 감정 해소, 일식 아크 엔딩)
- [ ] 사이드 대화 1~2종 (팀 유대 확인)
...
- [ ] 54스테이지 DPS/HP 커브 시뮬레이션 (7~15장)
- [ ] 그룹2 경제 시뮬레이션 (골드/코인 수급 균형)
- [ ] 신규 도구 2종 밸런스 검증
- [ ] 신규 셰프 2종 스킬 밸런스 검증
- [ ] 엔드리스 그룹2 적 통합 검증
- [ ] 스토리 연결성 검증 (그룹1→2 전환, 캐릭터 합류 시점, 대화 정합성)
...
| 22 | 8장 이자카야 심층부 + 7장 보스 재편 | ~126 | 20 | 📋 계획 |
| 23 | 9장 사케 오니 최종전 | ~136 | 20 | 📋 계획 |
