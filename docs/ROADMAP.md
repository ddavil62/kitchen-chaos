...
> 최종 업데이트: 2026-04-23 (Phase 74 UI/카피 마감 완료, 출시 Polish 로드맵 진행 중)
> 기준: Phase 74+75 완료 + 출시 Polish 로드맵(Phase 69~78) 계획
> 참고: 기존 로드맵 상 Phase 75 "일일 미션 + 로그인 보너스"는 BM/리텐션 트랙(P3)으로, 이번 2026-04-23의 Phase 75는 "행상인 분기 카드 풀 선행 해금 체크" 버그 수정(P2 트랙). 기존 일일 미션 계획은 Phase 75B 이후 번호로 이관 예정.
...
**현재 구현 완성도**: Phase 75 완료 (2026-04-23) — 1~24장 캠페인 전 스테이지 구현(placeholder 0개), 적 57종+보스 13종, 레시피 292종(일반 284+분기 8), 재료 32종, 셰프 7종 Named 전원 스킬 완성, 업적 34개. 전 캐릭터 64px chibi 아트 통일 완결. walk+death 애니메이션 시스템(ENEMY_IDS 42종 + 보스 13종 전종 등록). 미력의 정수+유랑 미력사 시스템 완성. 영업씬 3레이어 렌더링+챕터별 배경 완성(바닥 8종+뒷벽 8종). 엔드리스 확장: 미력 폭풍의 눈 이벤트(15웨이브 배수) + 정화 임무 4종(EndlessMissionManager) + 유랑 미력사 8% 등장 + 웨이브 구간별 배경 테마 전환 + 업적 6개 + 통계 트래킹(v21). 행상인 분기 카드 시스템: 4카테고리×8장=32장 전수 실효 완료, 변이 8종 전투 수치 반영 + Bond 8쌍 시너지 + Blessing 실효(골드·조리속도·인내심·코인·드롭·적 둔화) + 분기 레시피 반복 등장(chaos_ramen 3회/spice_bomb 2회)(v24). P0 전수 해결(67~68), 초반 UX 안전장치(69~70) 완료. 체커 패턴 복구 + 에셋 404 전수 수리(71) 완료. 분기 카드 수치 전수 반영(72) 완료. 세이브 3슬롯 롤링 백업+복구 UI + 포트레이트 정합(73) 완료. 행상인 분기 카드 선행 해금 체크(75, bond/mutation/recipe 필터 + chefUnlockHelper 공용화) 완료. UI/카피 마감 P2-4~7(74, 튜토리얼 페이지네이터 도트+셰프 실패 대사 21줄+도구 배지 3종+업적 glow+ShopScene cardH 112) 완료.
...
| **P3 BM/리텐션** | 75B~78 | 일일 미션, 손님 다양성, 셰프 스킨, 시즌/랭킹 (기존 Phase 75 "일일 미션" 번호 이관 예정) | 대기 |
...
#### Phase 75B (예정) — 일일 미션 + 로그인 보너스 (full, ui+art)

> 원래 Phase 75로 계획되었으나 2026-04-23 긴급 fix 배정으로 번호 이관.
