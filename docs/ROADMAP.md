...
| Phase 24 | 기존 8장→10장 번호 재편 + WorldMapScene 24챕터 확장 | ✅ 완료 |
...
### ✅ 21-1. 신규 재료/적 + 기믹 메커닉 (완료)

- [x] 재료 2종: 두부(tofu), 고수(cilantro) — 누적 19종
- [x] 적 2종: dumpling_warrior (HP 280, 분열 기믹), wok_phantom (HP 320, 화염 장판 기믹)
- [x] 분열 소환 적: mini_dumpling (HP 60, split:false)
- [x] 보스: dragon_wok (HP 7000, 화염 브레스, 3페이즈)
- [x] Enemy.js 특수 메커닉 3종: 분열(split), 화염 장판(fireZone), 화염 브레스+3페이즈(fireBreath)
- [x] GatheringScene _fireZones 배열 관리 + 도구 디버프 적용 + 3종 이벤트 핸들러
- [ ] PixelLab 스프라이트 7종 *(MCP 도구 미사용 가능, 생성 대기 중. fallback 정상 동작)*

### ✅ 21-2. 스테이지 8-1 ~ 8-6 (완료)

- [x] 6스테이지 웨이브 데이터 교체 (dumpling_warrior/wok_phantom/dragon_wok 구성)
- [x] SpriteLoader에 ENEMY_IDS +3, BOSS_IDS +1, TILESET_IDS +1, INGREDIENT_FILE_MAP +2 등록
- [x] 서비스 설정 (인내심 27~21초 구간)
- [x] 8-6 보스 스테이지: 최종 웨이브에 dragon_wok (count:1) + 사전 소환 파도

### ✅ 21-3. 레시피 확장 (10종, 완료)

- [x] 서빙 8종 (마파두부, 고수두부찜, 딤섬, 웍 볶음면, 두부 훠궈, 고수 탕수, 베이징 덕, 용의 만찬)
- [x] 버프 2종: dragon_qi (공격력 +30%), wok_aura (공속+공격력 +25%) *(buff_attack_speed -> buff_both로 대체)*
- [x] recipeData.js ALL_SERVING_RECIPES/ALL_BUFF_RECIPES에 등록 (gateStage 설정 포함)
- [x] 누적 126종 (서빙 102 + 버프 24)

### ✅ 21-4. 스토리 스크립트 (완료)

- [x] chapter8_intro (8-1 진입 시, 라오 고향 소개)
- [x] chapter8_lao_joins (8-3 첫 클리어 시, 라오 정식 합류)
- [x] chapter8_clear (8-6 첫 클리어 시, 드래곤 웍 정화 + chapter8_cleared 플래그 + currentChapter=9)
- [x] lao_side_8 (8장 클리어 후 행상인 진입 시, 라오 캐릭터 심화)
- [x] storyData.js 트리거 4건 + stage_first_clear 제외 목록에 8-1/8-3/8-6 추가

**구현 순서**: ~~21-1~~ → ~~21-2~~ → ~~21-3~~ → ~~21-4~~
...
## Phase 22-2 — 에셋 생성 (8장 신규 적)

> `visual_change: art` | PixelLab 스프라이트 생성
> 코드 구현 없음. 에셋만.

- [x] sake_specter: HP ~300, 마취 디버프 — PixelLab 생성
- [x] oni_minion: HP ~350, 돌진 — PixelLab 생성
- [x] 이자카야 지하 타일셋 — PixelLab tiles
- [x] 재료 아이콘: 사케(sake) 32px
...
## Phase 23-2 — 9장 스테이지 구현 (일식 아크 완성)

> `visual_change: none` | 게임 로직 구현
> 23-1 에셋 완성 후 진행.

- [ ] stageData.js: 스테이지 9-1 ~ 9-5 배치
- [ ] stageData.js: 9-6 sake_oni 보스전 배치 (HP 6500, 취기 장판+소환, 3페이즈)
- [ ] recipeData.js: 사케 조합 레시피 추가분 (누적 ~136종)
- [ ] 일식 아크 밸런스 점검 (7~9장 DPS 커브)
...
- [ ] CHAPTERS 배열: 12챕터 → 24챕터 구조
- [ ] 그룹1(1~6), 그룹2(7~15), 그룹3(16~24) 탭 UI
- [ ] 챕터 잠금 상태 SaveManager 연동

### 24-3. 테스트

- [ ] 기존 세이브 데이터 마이그레이션 검증
- [ ] 챕터 번호 참조 전수 조사
...
- [x] 54스테이지 DPS/HP 커브 시뮬레이션 (7~15장)
- [x] 그룹2 경제 시뮬레이션 (골드/코인 수급 균형)
- [x] 신규 도구 2종 밸런스 검증
- [x] 신규 셰프 2종 스킬 밸런스 검증
- [x] 엔드리스 그룹2 적 통합 검증
- [x] 스토리 연결성 검증 (그룹1→2 전환, 캐릭터 합류 시점, 대화 정합성)
...
## Phase 35 — 21장 엘 디아블로 최종전 (멕시칸 3장) ✅ 2026-04-16

> `visual_change: both` | 보스 스프라이트 + 스테이지

- [x] el_diablo_pepper 보스 스프라이트 (PixelLab pro, 64px, 8방향 + walking 애니메이션)
- [x] dialogueData.js: 대화 6종 추가 (chapter21_intro, chapter21_mid, chapter21_boss, chapter21_clear, chapter21_epilogue, team_side_21) — 누적 94종
- [x] el_diablo CHARACTERS 추가 (portraitKey: 'el_diablo', boss 역할)
- [x] storyData.js: 트리거 7건 추가 (21장 전체 스토리 흐름) — 누적 96항목
- [x] stageData.js: 스테이지 21-1~21-6 구현 (21-1~21-5 일반, 21-6 보스전, theme: cactus_cantina)
- [x] recipeData.js: 레시피 10종 추가 (diablo 테마 특선 + 아보카도 조합) — 누적 254종
- [x] el_diablo_pepper 보스 등록: HP 2600, speed 22, fireZone + 소환 + 분노 메커닉
- [x] SpriteLoader.js: BOSS_IDS 12종 갱신, BOSS_WALK_HASHES el_diablo_pepper 등록
- [x] 멕시칸 아크(19~21장) 3챕터 완결

---

## Phase 36 — 22장 슈가 드림랜드 (디저트 1장) ✅ 2026-04-16

> `visual_change: both` | 스크립트 + 에셋 + 스테이지

- [x] dialogueData.js: chapter22_intro (드림랜드 진입), chapter22_mid (미각의 여왕 존재 암시), team_side_22 (사이드 대화) — 누적 97종
- [x] candy_soldier (HP 450, damageReduction 20%), cake_witch (HP 500, summon+소환 60%) — PixelLab standard 48px 8방향 + walking 애니메이션
- [x] 재료 아이콘 2종: cacao(카카오), vanilla(바닐라) 32px RGBA — 누적 31종
- [x] storyData.js: 트리거 4건 추가 (22-1 진입/22-3 클리어/22-5 클리어/merchant_enter) — 누적 100항목
- [x] stageData.js: 스테이지 22-1~22-5 구현 (theme: dream_candy, candy_soldier+cake_witch 웨이브)
- [x] recipeData.js: 카카오+바닐라 조합 레시피 10종 (서빙 8종 + 버프 2종) — 누적 264종
- [x] SpriteLoader.js: ENEMY_IDS 41종 갱신, ENEMY_WALK_HASHES candy_soldier/cake_witch 등록
- [x] 디저트 아크(22~24장) 시작
...
- [ ] 54스테이지 DPS/HP 커브 시뮬레이션 (16~24장)
- [ ] 전체 144스테이지 경제 균형 최종 점검
- [ ] 최종 보스 3페이즈 밸런스 집중 검증
- [ ] 엔드리스 그룹3 적 통합
- [ ] 전체 스토리 흐름 최종 검토
...
> 전체 구조: 그룹1(챕터 1~6 완료) → 그룹2(챕터 7~15, Phase 22~30) → 그룹3(챕터 16~24, Phase 31~40)
...
## Phase 42 — 아트 리워크: 레거시 스프라이트 64px 재생성
...
- 실제 렌더 크기: 일반 적 35px, 보스 50px (게임 캔버스 360x640)
