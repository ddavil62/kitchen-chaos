...
#### Phase 77 — 엔드리스 해금 조건 완화 + 세션 속도 버튼 (quick, ui)
...
**목표**:
- 엔드리스 해금 조건을 `6-6 클리어`로 완화 (WorldMapScene + MenuScene 상수 변경)
- GatheringScene에 `2× 배속` 버튼 추가 (웨이브 진행 중 상시 토글)
- 배속 상태에서 wave/enemy/projectile update delta 배수 적용

**구현 범위**:
- `WorldMapScene.js` / `MenuScene.js`: `ENDLESS_UNLOCK_STAGE = '24-6'` → `'6-6'` 변경, 잠금 문구 갱신
- `GatheringScene.js`: `_speedMult` 상태값 + 배속 버튼 토글 UI (2× 아이콘, 우상단 배치)
- `Enemy.js` / `GatheringScene.update()`: `dt * this._speedMult` 적용

**의존성**: 없음.

---

#### Phase 78 — 이진 실패 → 부분 성공 구조 전환 (full, none)
...
HP = 0, 재료 > 0 (부분 실패): 수집 재료의 50%만 영업에 사용. 별점 최대 ★★☆
