...
> 최종 업데이트: 2026-04-16 (Phase 34 완료)
...
kitchen-chaos/
  js/
    ...
    data/
      gameData.js            # 적(50종)/도구(TOOL_DEFS)/재료(29종) 정의
      stageData.js           # 스테이지 데이터 138슬롯 (구현: 1~7/9~13/15~20장, placeholder: 8/14/20-6/21~24장)
      recipeData.js          # 레시피 244종 정의 (서빙 200 + 버프 44 추정)
      dialogueData.js        # 대화 스크립트 88종 + 캐릭터 13종 정의 (시즌2 7~15장 31종, 16장 4종, 17장 3종, 18장 6종, 19장 3종, 20장 3종 포함)
      storyData.js           # STORY_TRIGGERS 트리거 데이터 89항목 (triggerPoint 8종, import SaveManager)
...
| 대화 시스템 | DialogueManager.js + DialogueScene.js + dialogueData.js | 대화 스크립트 88종 재생, 선택지 분기 UI, 픽셀아트 초상화 렌더링, 시청 기록 |
| 스토리 시스템 | StoryManager.js + storyData.js | 트리거 중앙 디스패처(triggerPoint 8종), 89항목, 챕터 진행도, 스토리 플래그(객체), onComplete 콜백, 씬 1줄 호출 |
...
| 캠페인 | 24챕터 체계(그룹1~3), 구현 완료 1~7/9~13/15~20장, 보스 11종, 별점 시스템 | 완료 |
| 레시피 | 244종, 5등급, 도감 | 완료 |
...
| 대화/스토리 | 스크립트 88종, 트리거 89항목, 선택지 분기, 초상화 6종, 13캐릭터 | 완료 |
...
| 20장 칸티나 심층부 (Phase 34) | 대화 3종, 적 2종(cactus_wraith/luchador_ghost), 재료 1종(avocado), 레시피 10종(+1추정), 스테이지 20-1~20-5, thornReflect/taunt+dodge 메커닉 | 완료 |

## 콘텐츠 규모

| 항목 | 수량 |
|------|------|
| 적 | 50종 (일반 38 + 미니보스 1 + 보스 11) |
| 도구 | 8종 (pan, salt, grill, delivery, freezer, soup_pot, wasabi_cannon, spice_grinder) |
| 재료 | 29종 |
| 레시피 | 244종 |
| 스테이지 | 138슬롯 (구현 완료: 1~7/9~13/15~20장, placeholder: 8/14/20-6/21~24장) |
...
- Phase 35~: 그룹3(21~24장) 확장. 상세: `docs/ROADMAP.md`
