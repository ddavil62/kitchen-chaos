...
| 영업 코어 | ServiceScene.js | 손님 입장/주문/조리/서빙/팁, 골드→영구 저장, 픽셀아트 스프라이트 렌더링 (fallback 지원) |
...
| 영업 씬 비주얼 | ServiceScene 픽셀아트 렌더링 (테이블 5종, 손님 5종, 홀 바닥, 조리 카운터) + fallback | 완료 |
...
- Phase 19 완료 (시즌2 기반 인프라 -- 데이터/UI/에셋/스토리/영업 씬 비주얼 리워크).
