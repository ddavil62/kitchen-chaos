...
> 최종 업데이트: 2026-04-26 (TavernServiceScene 현대식 레스토랑 Phase G 완료 -- 6테이블 24석)
...
### 영업씬 태번(Travellers Rest) 스타일 재설계 (Phase A ~ G 완료)

Travellers Rest 식 탑다운 가구 + 사이드뷰 풀바디 캐릭터로 영업씬 전면 재설계. Phase G에서 v13 현대 캐주얼 다이닝 가구로 전면 교체하고, 2열x3행 6테이블 24석 레이아웃으로 확장. front=seated_south(정면), back=seated_north(뒷모습) depth 착석 표현. 35/35 Playwright PASS.

- **신규 파일**: `js/scenes/TavernServiceScene.js`, `js/data/tavernLayoutData.js`, `js/data/tavernStateData.js`
- **실 에셋**: `assets/tavern/` (v13 가구 3종 + 손님 seated_south/north 20종 + 손님 walk 20종 + 셰프 idle 7 + 셰프 walk 10 + 레거시 가구)
...
- **REAL_KEY_MAP**: v13 가구 3종 + 손님 seated_south/north 20종 + 셰프 2종 + normal 2종
...
### Phase G -- TavernServiceScene 현대식 레스토랑 리디자인 (2026-04-26)

v12(중세 태번) 가구를 v13(현대 캐주얼 다이닝)으로 전면 교체. 테이블 배치 1열x2행(2테이블, 12석) -> 2열x3행(6테이블, 24석) 확장. 손님 seated_south/north 20종 현대 스타일 교체(64x64px).

- QA: PASS (35건 전부 통과)
- AD 모드2: APPROVED, AD 모드3: APPROVED
- 스펙 대비 차이: 손님 에셋 크기 스펙 48px -> 실제 64px 유지(walk 동일 크기, 픽셀 계단 방지)
- 잔존 이슈: v12 세로 가구 preload 잔존(LOW), occupySlot JSDoc 범위 표기 미갱신(LOW)
- 스펙: `.claude/specs/2026-04-26-kc-phase-g-spec.md`

Phase F 이전 이력은 [CHANGELOG.md](CHANGELOG.md) 및 [SERVICE_SCENE_TAVERN_PHASES.md](SERVICE_SCENE_TAVERN_PHASES.md) 참조.
