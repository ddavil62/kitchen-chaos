...
> 최종 업데이트: 2026-04-28 (Phase 85 일일 미션 풀 확장 완료)
...
| 일일 미션 | DailyMissionManager.js | DAILY_MISSION_POOL 20종에서 매일 3개 랜덤 선정, 자정 리셋, 진행도 추적(누적 합산+max 갱신), 보상 자동 지급 |
...
| 세이브 | SaveManager.js | localStorage, 마이그레이션 체인 v1~v28, 3슬롯 롤링 백업(backup_1~3) + getBackups/restoreBackup API, dailyMissions/loginBonus/mimiSkinCoupons/unlockedSkins/equippedSkin/regularCustomerProgress/criticPenaltyActive 필드, `_currentRun` 인메모리 단일 소스 (씬 간 stageId 전달) |
...
| 일일 미션 | DailyMissionManager: 20종 풀(6타입+신규 3타입)에서 매일 3개 랜덤 선정, 9개 씬 이벤트 훅(try-catch), 자정 리셋, 보상 자동 지급. 메뉴 배너 + 통합 팝업 모달 | 완료 |
...
### Phase 85 -- 일일 미션 풀 확장 (2026-04-28)

DAILY_MISSION_POOL 10종 → 20종 확장. 신규 미션 타입 3가지(vip_serve, combo_reach, gold_single_run) 추가. ServiceScene에 미션 연동 훅 3종 삽입. recordProgress max 갱신 분기에 combo_reach/gold_single_run 추가.

- QA: PASS (15건, 코드 버그 0건)
- 스펙: `.claude/specs/2026-04-28-kc-phase85-scope.md`

### Phase 84 -- 셰프 스킨 시스템 (2026-04-28)

SkinManager 신규 생성(미미 스킨 3종: 기본/핑크/블루 앞치마), ChefSelectScene에 스킨 선택 서브 패널 UI 추가, IAPManager에 스킨 구매 스텁 추가, SaveManager v28 마이그레이션(unlockedSkins/equippedSkin). portrait 에셋 2종 PIL 팔레트 교체 생성.

- QA: PASS (40건, 32 통과 / 8 실패는 테스트 인프라+기존 이슈, 코드 버그 0건)
- AD 모드 2: APPROVED, AD 모드 3: APPROVED
- 스펙: `.claude/specs/2026-04-28-kc-phase84-planner.md`
