...
> 최종 업데이트: 2026-04-25 (영업씬 태번 가구 비례 업스케일 Phase B-6-2 완료)
...
| 영업 씬 비주얼 | 아이소메트릭 홀 (3레이어 분리 렌더링, 테이블 front/back 10종+손님 10종+챕터별 바닥 8종+뒷벽 8종, 홀 데코, depth 체계 정비), 태번 가구 3종 비례 업스케일(bench 28x96, table 44x96) + 좌석 anchor 재정렬 | 완료 |
...
### 영업씬 태번(Travellers Rest) 스타일 재설계 (Phase A + A-bis + B-1 ~ B-6-2 완료)

Travellers Rest 식 탑다운 가구 + 사이드뷰 풀바디 캐릭터로 영업씬 전면 재설계 진행 중. Phase B-6에서 캐릭터 32x48 업스케일 완료, Phase B-6-2에서 가구 3종(bench 14x76->28x96, table 44x72->44x96) 비례 업스케일 및 좌석 anchor 재정렬 완료. 184/184 Playwright PASS.

- **신규 파일**: `js/scenes/TavernServiceScene.js`, `js/data/tavernLayoutData.js`, `js/data/tavernStateData.js`
- **실 에셋**: `assets/tavern/` (62종 PNG: 가구 5 + 손님 seated 20 + 손님 walk 20 + 셰프 idle 7 + 셰프 walk 10), 에셋 규격 32x48(정지)/128x48(walk 4프레임), 가구 규격 bench 28x96/table 44x96, PIL placeholder: `assets/tavern_dummy/` (보존)
- **레거시 백업**: `assets/tavern/.legacy-b5/` (57 PNG, 기존 16x24 에셋), `assets/tavern/.legacy-b6-2/` (가구 3종, 기존 bench 14x76 + table 44x72)
- **진입점**: `?scene=tavern` URL 파라미터 (디버그 전용)
- **현재 레이아웃**: V12 (4분면 세로 테이블, quad 104x120px, ASSET_MODE='real')
...
### Phase B-6-2 -- 태번 가구 비례 업스케일 및 좌석 anchor 재정렬 (2026-04-25)

가구 3종을 PIL 픽셀 확장으로 업스케일(bench 14x76->28x96, table 44x72->44x96). tavernLayoutData.js 상수 전면 갱신(QUAD_W 100->104, BENCH_W 14->28 등). 좌석 anchor 및 BENCH_SLOTS.lv0 dy 재정렬. 기존 가구 3종은 `.legacy-b6-2/`에 백업.

- QA: PASS (184/184 -- 신규 48 + 회귀 136)
- AD 모드2: APPROVED
- AD 모드3: APPROVED with NOTE (bench-r/table 4px 겹침, tr/br quad DINING_RIGHT 4px 초과, 하단 여백 190px -- 모두 비파괴적)
- 잔존 이슈: lv4 slot0 dy=10 < bench top=12 (미래 확장 시 재정렬 필요)
- 스펙: `.claude/specs/2026-04-25-kc-phase-b6-2-scope.md`

Phase B-6 이전 이력은 [CHANGELOG.md](CHANGELOG.md) 및 [SERVICE_SCENE_TAVERN_PHASES.md](SERVICE_SCENE_TAVERN_PHASES.md) 참조.
