...
> 최종 업데이트: 2026-04-28 (Phase 81 리워드 광고 + IAP 수익화 기반 구조 완료)
...
| 결과 | ResultScene.js | 캠페인 별점/엔드리스 기록 표시, 행상인 방문 연결, modal lock (DialogueScene 오버레이 시 버튼 비활성), isCleared 복합 조건, 셰프별 장보기 실패 대사 (7셰프 x 3 바리에이션), BTN_W=240, partialFail 별점 2개 캡+오렌지 메시지, AD-1 광고 재도전(완전/부분 실패)+AD-3 보상 2배(stars<=2) 버튼, btnGap 조건부 축소(44/54) |
...
| 메인 메뉴 | MenuScene.js | 배경 이미지(menu_bg) + 타이틀 로고(menu_title_logo) + 미미 스프라이트 + 버튼 5종 + 설정/쿠폰/세이브 복구 + 오늘의 미션 배너 + 미션/캘린더 통합 모달, 미션 진행도 바 barH=16, 쿠폰 팝업 alpha=0.75, 세이브 팝업 depth=1300 |
...
### Phase 80 -- 영업 씬 서빙 인터랙션 강화 (2026-04-28)

ServiceScene에 3종 시각 피드백 추가. 레시피 탭 시 해당 주문 테이블에 골든 테두리 강조(2단계 서빙), patience<30% 테이블에 빨간 깜빡임, 2+콤보 달성 시 화면 중앙 팝업 텍스트.

- QA: PASS
- AD 모드 3: APPROVED (FAIL 0, WARN 7 전부 허용)
