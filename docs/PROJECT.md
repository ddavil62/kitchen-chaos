...
> 최종 업데이트: 2026-04-28 (Phase 80 영업 씬 서빙 인터랙션 강화 완료)
...
| 모듈 | 파일 | 역할 |
|------|------|------|
| 재료 채집 | GatheringScene.js | 도구 배치/회수/재배치, 적 AI, 재료 드롭, 보스 재료 드롭, 웨이브 진행, 1x/2x 배속 토글, HP=0 부분 성공 분기 |
...
| 결과 | ResultScene.js | 캠페인 별점/엔드리스 기록 표시, 행상인 방문 연결, modal lock (DialogueScene 오버레이 시 버튼 비활성), isCleared 복합 조건, 셰프별 장보기 실패 대사 (7셰프 x 3 바리에이션), BTN_W=240, partialFail 별점 2개 캡+오렌지 메시지 |
...
### Phase 79 -- 인터랙티브 온보딩 재설계 (2026-04-28)

1-1 튜토리얼을 텍스트 팝업 방식에서 화살표+하이라이트 overlay 3스텝 방식으로 전환. 스킵 버튼 hitArea 72x44px 확보. 튜토리얼 웨이브 적 수 2마리 고정(0.6 압축 면제).

- QA: PASS (23건, Playwright 인프라 이슈 5건은 코드 결함 아님)
