...
> 최종 업데이트: 2026-04-24 (영업씬 태번 스타일 Phase B-5-1 셰프 5명 walk 스프라이트시트 10장 발주 완료)
...
### 영업씬 태번(Travellers Rest) 스타일 재설계 (Phase A + A-bis + B-1 ~ B-4 + B-5-1 완료)

Travellers Rest 식 탑다운 가구 + 사이드뷰 풀바디 캐릭터로 영업씬 전면 재설계 진행 중. Phase B-5-1에서 셰프 5명(mage/yuki/lao/andre/arjun) walk_l/walk_r 4프레임 스프라이트시트 10장을 PixelLab animate_character로 발주 완료. B-5 원래 범위(셰프 포즈+가구+환경물)에서 AD1 권고로 walk만 발주, carry/cook/serve는 B-5-1b로, 가구는 B-5-2로, 환경물은 B-5-3으로 분리. 223/223 Playwright PASS (B-5-1 신규 39 + 회귀 B-1~B-4 100).

- **신규 파일**: `js/scenes/TavernServiceScene.js`, `js/data/tavernLayoutData.js`, `js/data/tavernStateData.js`
- **실 에셋**: `assets/tavern/` (62종 PNG: 가구 5 + 손님 seated 20 + 손님 walk 20 + 셰프 idle 7 + 셰프 walk 10), PIL placeholder: `assets/tavern_dummy/` (보존)
...
### Phase B-5-1 — 셰프 5명 walk 스프라이트시트 10장 발주 (2026-04-24)

셰프 5명(mage/yuki/lao/andre/arjun) walk_r/walk_l 4프레임 스프라이트시트 10장(각 64x24px)을 PixelLab animate_character로 발주. AD1에서 B-5 원래 범위(walk+carry+cook+serve 최대 35장)를 walk만(10장)으로 축소 결정. carry/cook/serve는 B-5-1b로 분리. TavernServiceScene.js에 spritesheet preload 10개 + Phaser anims 10개(8fps, repeat=-1) + C/V 데모 키 핸들러 + window.__tavernChefAnims 진단 노출 추가.

- QA: PASS (B-5-1 신규 23 + 에지 16 = 39/39, 회귀 B-1~B-4 100/100 PASS)
- AD 모드2: APPROVED (walk 10/10 PASS)
- 알려진 한계: _chefs[0]가 Image 객체이므로 C/V 키 walk 미재생 (B-4 동일, Phase D Sprite 전환 시 해소)
...
Phase B-2 이전 이력(B-2/B-1/A-bis/A)은 [CHANGELOG.md](CHANGELOG.md) 및 [SERVICE_SCENE_TAVERN_PHASES.md](SERVICE_SCENE_TAVERN_PHASES.md) 참조.
