...
> 최종 업데이트: 2026-04-16 (Phase 36 완료)
...
kitchen-chaos/
  js/
    ...
    data/
      gameData.js            # 적(53종)/도구(TOOL_DEFS)/재료(31종) 정의
      stageData.js           # 스테이지 데이터 138슬롯 (구현: 1~7/9~13/15~22장, placeholder: 8/14/22-6~24장)
      recipeData.js          # 레시피 264종 정의
      dialogueData.js        # 대화 스크립트 97종 + 캐릭터 14종 정의 (시즌2 7~15장 31종, 16장 4종, 17장 3종, 18장 6종, 19장 3종, 20장 3종, 21장 6종, 22장 3종 포함)
      storyData.js           # STORY_TRIGGERS 트리거 데이터 100항목 (triggerPoint 8종, import SaveManager)
  assets/                    # 스프라이트/타일셋/아이콘 (PixelLab 픽셀아트)
    sprites/portraits/       # 캐릭터 초상화 6종 (64x64 PixelLab)
    sprites/chefs/           # 셰프 스프라이트 5종 (48px)
    sprites/enemies/         # 적 스프라이트 41종 (masala_guide 108px, taco_bandit 160px, burrito_juggernaut 172px, cactus_wraith, luchador_ghost, candy_soldier, cake_witch 포함)
    ...
    icons/                   # 재료 아이콘 31종 (curry_leaf, saffron, chai, cardamom, jalapeno, avocado, cacao, vanilla 32px 포함)
...
| 대화 시스템 | DialogueManager.js + DialogueScene.js + dialogueData.js | 대화 스크립트 97종 재생, 선택지 분기 UI, 픽셀아트 초상화 렌더링, 시청 기록 |
| 스토리 시스템 | StoryManager.js + storyData.js | 트리거 중앙 디스패처(triggerPoint 8종), 100항목, 챕터 진행도, 스토리 플래그(객체), onComplete 콜백, 씬 1줄 호출 |
...
| 캠페인 | 24챕터 체계(그룹1~3), 구현 완료 1~7/9~13/15~22장(22-1~22-5), 보스 12종, 별점 시스템 | 완료 |
| 레시피 | 264종, 5등급, 도감 | 완료 |
...
| 대화/스토리 | 스크립트 97종, 트리거 100항목, 선택지 분기, 초상화 6종, 14캐릭터 | 완료 |
...
- Phase 37~: 그룹3(23~24장) 확장. 상세: `docs/ROADMAP.md`
