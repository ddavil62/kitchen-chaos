...
> 최종 업데이트: 2026-04-28 (Phase 82 MenuScene 리소스 HUD + 별점 보상 코인 정책 정비 완료)
...
| 모듈 | 파일 | 역할 |
|------|------|------|
| 재료 채집 | GatheringScene.js | 도구 배치/회수/재배치, 적 AI, 재료 드롭, 보스 재료 드롭, 웨이브 진행, 1x/2x 배속 토글(AD-2 광고 선행), HP=0 부분 성공 분기, overrideLives 파라미터 지원 |
...
| IAP 관리 | IAPManager.js | 광고 제거 IAP 스텁. localStorage `kc_ads_removed` 기반 상태 조회. 실제 결제 플로우 미구현 |
| 세이브 | SaveManager.js | localStorage, 마이그레이션 체인 v1~v27, 3슬롯 롤링 백업(backup_1~3) + getBackups/restoreBackup API, dailyMissions/loginBonus/mimiSkinCoupons/regularCustomerProgress/criticPenaltyActive 필드, `_currentRun` 인메모리 단일 소스 (씬 간 stageId 전달) |
...
| 스프라이트 | SpriteLoader.js | walk/death 프레임 시퀀스 로딩 (적+보스+미니보스), Phaser anim 등록, 방향 폴백 매핑, 챕터별 홀 바닥·뒷벽 에셋 로드, 테이블 front/back+손님 waiting/seated(10종 프로필, seated_south+seated_north) 에셋 로드, 타일셋 15종+타워 8종 preload, portrait 9종 (arjun 포함) |
| 메인 메뉴 | MenuScene.js | 배경 이미지(menu_bg) + 타이틀 로고(menu_title_logo) + 미미 스프라이트 + 버튼 5종 + 설정/쿠폰/세이브 복구 + 오늘의 미션 배너 + 미션/캘린더 통합 모달 + 리소스 HUD(골드/코인/미력의 정수), 미션 진행도 바 barH=16, 쿠폰 팝업 alpha=0.75, 세이브 팝업 depth=1300 |
...
| 메뉴 리소스 HUD | MenuScene 상단(y=100)에 골드/코인/미력의 정수 3열 상시 표시. 재클리어 코인 0 정책 + ResultScene 재클리어 문구 | 완료 |
...
- 미미 스킨 쿠폰(mimiSkinCoupons)은 카운터만 저장, 교환/사용 UI 미구현
...
### Phase 82 -- MenuScene 리소스 HUD + 별점 보상 코인 정책 정비 (2026-04-28)

MenuScene 상단(y=100)에 골드/코인/미력의 정수 3열 텍스트 HUD 추가. SaveManager clearStage() 재클리어 분기 코인 지급을 0으로 변경 (기존 20% 소량 지급 제거). ResultScene에서 재클리어 시 "재클리어 (추가 보상 없음)" 문구 표시.

- QA: PASS (33건, flaky 2건은 테스트 인프라 이슈)
- AD 모드 3: APPROVED
- 스펙: `.claude/specs/2026-04-28-kc-phase82-planner.md`

### Phase 81 -- 리워드 광고 + IAP 수익화 기반 구조 (2026-04-28)

AdManager.js + IAPManager.js 신규 생성. 리워드 광고 3포인트 삽입: AD-1(완전/부분 실패 재도전, HP=8), AD-2(1x->2x 배속 광고 선행), AD-3(stars<=2 보상 2배, 1회 제한). DEV/웹 환경 즉시 리워드 폴백. BootScene 초기화 연동.

- QA: PASS (27건, 코드 버그 0건)
